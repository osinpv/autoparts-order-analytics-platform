...
def test_release_order_from_new_status_fails(client, db_session):
    setup = create_order_setup(
        client,
        sku="BRK-008",
        product_name="Release Wrong Status Pads",
        price="69.99",
        warehouse_code="NEWREL",
        warehouse_name="New Release Warehouse",
        region="Florida",
        on_hand_qty=10,
        reserved_qty=0,
        order_number="SO-400001",
    ...
    release_response = client.post(f"/orders/{order['order_id']}/release")
    assert release_response.status_code == 400
    assert release_response.json()["detail"] == "Only orders in RESERVED status can be released."
